...
5. Para cada tarjeta con `gasto_minimo_mensual` configurado, el sistema calcula el ciclo de corte
   ...
   mínimo.
6. El sistema muestra las cards (imagen configurada, nombre y balance centrados, y al pie la barra
   de utilización de línea de crédito con su % y disponible) — 4 o 5 visibles, resto en carrusel. El
   avance de gasto mínimo no se muestra por ahora (RN-237).
...
- RN-237: El gasto del ciclo de una tarjeta es la suma, en valor absoluto, de los montos de
  ...
  contra `gasto_minimo_mensual` para conocer el avance del ciclo.
  **Su presentación está pendiente de definir (2026-09-06).** El cálculo sigue vigente y disponible
  (`use-credit-cards-cycle-spend`), pero el dato **no se muestra hoy en ninguna pantalla**: se probó
  como segunda barra de progreso y como chip junto al saldo en la card de tarjeta (CU-063), y ambas
  se retiraron. El criterio que dejó ese intento y que debe respetar la solución definitiva es que la
  card mida y se estructure igual tenga o no `gasto_minimo_mensual` configurado — solo algunas
  tarjetas lo tienen, y colgar de él un bloque entero hacía que unas cards se estructuraran distinto
  de otras dentro de la misma cuadrícula.
