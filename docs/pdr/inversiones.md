...
- RN-162: `diferencia` de un instrumento activo = `balance_actual − ((porcentaje_objetivo ÷ 100) ×
  ...
  aportación.
- RN-328: las columnas de diagnóstico llevan formato condicional con una tolerancia del **20% del
  porcentaje objetivo**, medida en proporción a ese objetivo y no en puntos porcentuales. Con
  objetivo de 10%, `porcentaje_actual` se muestra en rojo hasta 8%, sin color entre 8% y 12%, y en
  verde a partir de 12%; con objetivo de 3%, los mismos límites caen en 2.4% y 3.6%. La tolerancia
  existe porque una cartera nunca cuadra al decimal, y pintar cada desviación mínima haría que el
  color dejara de significar algo.
  Es proporcional y no fija porque un umbral en puntos porcentuales se vuelve ciego justo donde el
  desvío duele más: con dos puntos de tolerancia, un instrumento de objetivo 3% tendría que perder
  dos tercios de su posición antes de pintarse, mientras que uno de objetivo 25% se pintaría por una
  desviación proporcionalmente ocho veces menor. Medido contra el objetivo, el umbral significa lo
  mismo en toda la tabla.
  El `porcentaje_nuevo` de la simulación (CU-053) usa el mismo umbral pero **un solo color de
  advertencia** en vez de rojo y verde: ahí la pregunta es si la aportación acerca el instrumento a
  su objetivo, y quedarse corto o pasarse son el mismo problema. `diferencia` muestra el signo
  siempre, también en positivo, y la aportación sugerida en cero se atenúa para que solo resalten las
  filas que sí reciben dinero.
- RN-329: ambas tablas del portafolio se pueden ordenar por ticker alfabético, por porcentaje
  objetivo o por balance, en las dos direcciones. Cada tabla conserva su propio orden —son dos
  listados independientes— y el valor por omisión es ticker ascendente, el mismo que ya devolvía la
  consulta. El ordenamiento por objetivo no se ofrece en la tabla de inactivos, donde esa columna no
  existe (RN-163). Los empates se resuelven por ticker, para que dos instrumentos con el mismo valor
  no intercambien lugar entre renders.
...
- RN-330: el desglose se presenta como **una sola barra apilada** con su leyenda, no como una barra
  por categoría: lo que se quiere leer es el reparto, y con barras separadas hay que compararlas de a
  dos con la vista. Cada categoría tiene un color fijo definido en la aplicación —no asignado por
  posición— para que conserve su identidad aunque cambie el orden de la barra al moverse los
  balances; los tonos se eligen alejados entre sí en el círculo cromático, porque los segmentos se
  tocan y dos matices vecinos se leerían como uno solo. `Crypto` comparte color en ambos catálogos,
  al ser la misma cosa vista por dos clasificaciones. Una categoría sin balance no aparece.
...
|---|---|---|---|
| 2026-09-06 | Ordenamiento en las tablas del portafolio (RN-329) y rediseño del desglose de exposición (RN-330), sin cambios de esquema ni de cálculo. Las tablas se ordenan por ticker, objetivo o balance en ambas direcciones, cada una con su propio estado. El desglose pasa de una barra por categoría a una sola barra apilada con leyenda, con color fijo por categoría en vez del color único que tenía. | CU-050, CU-051 | Se actualiza [[data-model-registry]]: índice de numeración hasta RN-330 |
| 2026-09-06 | Formato condicional en las columnas de diagnóstico del portafolio (RN-328), sin cambios de esquema ni de cálculo: los valores mostrados son los mismos, solo se colorean. Se fija una tolerancia del 20% del porcentaje objetivo, **proporcional y no en puntos porcentuales** — un umbral fijo se volvía ciego en los objetivos pequeños, donde un instrumento de objetivo 3% habría tenido que perder dos tercios de su posición antes de pintarse. La comparte `porcentaje_actual` (rojo por debajo, verde por encima) con el `porcentaje_nuevo` de la simulación, que usa un único color de advertencia porque ahí la dirección del desvío es indiferente. `diferencia` pasa a mostrar el signo también en positivo y la aportación sugerida en cero se atenúa. | CU-050, CU-053 | Se actualiza [[data-model-registry]]: índice de numeración hasta RN-328 |
