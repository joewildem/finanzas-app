...
|Reglas de negocio (RN-XXX)|RN-331|dashboard|
...
> Nota (2026-08-28): `docs/pdr/dashboard.md` (pestaña Analytics, CU-069 a CU-071, RN-257 a RN-269), la última pestaña del Dashboard, sucede funcionalmente a `docs/pdr/reportes.md` (CU-027, CU-028, CU-030 — resumen por grupo, distribución de gasto por categoría, ingresos vs. gastos), con una corrección estructural: el viejo cálculo de ingresos vs. gastos (RN-094, retirado) excluía el grupo "Investment" del gasto por nombre exacto — Analytics resuelve esto con el nuevo valor estructural `categories.flujo = investment` (ver nota de `categorias` más abajo), en vez de perpetuar la comparación por nombre. **CU-031** (frecuencia de transacciones) de [[reportes]] no se retoma — queda en [[backlog]] como posible extensión futura, fuera del alcance descrito para Analytics. Los números `CU-023`–`CU-031` y `RN-076`–`RN-097` de [[reportes]] no se reutilizan ni se renumeran — quedan como registro histórico completo de un módulo cuya navegación se retiró de la aplicación sin construirse en su forma original. Con este cierre, [[reportes]] queda enteramente resuelto (Balance, Networth y Analytics cubren la totalidad de lo que ese módulo pretendía).
...
|`flujo`|text\|null (enum `category_flow`: `inflow`, `outflow`, `investment`)|Sí si `tipo=grupo`; `null` si `tipo=categoria`|Sin default — elección explícita|CU-007 (RN-118); editable en CU-010; las categorías heredan el flujo de su grupo en tiempo de consulta, no lo almacenan. `investment` agregado 2026-08-28 al alinear [[dashboard]] (Analytics) — antes solo `inflow`/`outflow`|
|`orden`|smallint\|null|Sí si `tipo=grupo`; `null` si `tipo=categoria`|Siguiente posición disponible|CU-007 (RN-119); reordenable en CU-009 (controles ↑/↓); una sola secuencia compartida entre todos los flujos|
...
> Nota (actualizada 2026-08-23): `grupo_id` se diseñó pensando en reutilizarse en Ahorros, Créditos e Inversión, pero dos de los tres módulos optaron por tabla propia: Ahorros con `savings_goals` (ver [[ahorros-y-metas]]) e Inversiones con `investments` (ver [[inversiones]], donde grupo y tipo de activo son dos taxonomías ortogonales que no caben en una jerarquía de un nivel). La reutilización directa sigue vigente como opción abierta únicamente para Créditos y Deudas. Nótese que el **grupo de categorías** llamado "Investment" sí sigue existiendo y en uso: es donde se clasifica el gasto que representa capital entrando al portafolio (RN-025 de [[categorias]]) — no debe confundirse con el módulo [[inversiones]], que no lo referencia salvo para leer su presupuesto mensual (RN-171). **Actualizada 2026-08-28:** desde que `flujo` ganó el valor `investment` (RN-118 de [[categorias]]), este grupo ya no depende de su nombre exacto para distinguirse del resto de Outflow — es estructural.

> Nota (2026-08-28): `categories.flujo` (enum `category_flow`) gana un tercer valor, `investment`, junto a `inflow`/`outflow` — ver RN-118 (revisada) en [[categorias]]. Reemplaza la última comparación por nombre exacto que quedaba pendiente en el sistema (el chip "Investment" del formulario de alta de transacciones, y el cálculo de ingresos vs. gastos del viejo [[reportes]], ya retirado). Cambio cruzado en [[presupuesto]] (tercera tabla "Investment", RN-075) y [[transacciones]] (RN-039, `create_transaction`/`update_transaction` aceptan `gasto` contra `outflow` o `investment`).
...
|2026-09-06|inversiones|Formato condicional en las columnas de diagnóstico del portafolio (RN-328). **Sin cambios de esquema ni de cálculo**: ninguna colección, índice ni relación se modifica, y los valores mostrados son los mismos — solo se colorean. Se fija una tolerancia del 20% del porcentaje objetivo —proporcional, no en puntos porcentuales, porque un umbral fijo se volvía ciego en los objetivos pequeños—, compartida por `porcentaje_actual` y por el `porcentaje_nuevo` de la simulación de aportación; el primero distingue por debajo (rojo) de por encima (verde), el segundo usa un solo color de advertencia porque ahí la dirección del desvío es indiferente. Se actualiza el índice hasta RN-328.|CU-050, CU-053|Ninguno|
|2026-09-06|inversiones|Ordenamiento en las tablas del portafolio (RN-329) y rediseño del desglose de exposición (RN-330). **Sin cambios de esquema ni de cálculo**: ninguna colección, índice ni relación se modifica. El desglose pasa a una barra apilada única con color fijo por categoría —definido en la aplicación, no asignado por posición, para que una categoría conserve su identidad cuando cambia el orden— y `Crypto` comparte color entre `grupo_activo` y `tipo_activo` al ser la misma cosa vista por dos clasificaciones. Se actualiza el índice hasta RN-330.|CU-050, CU-051|Ninguno|
|2026-09-06|dashboard|Ajuste visual de las cards de cuenta (RN-331, RN-237 revisada). **Sin cambios de esquema ni de cálculo.** Las cards de débito/efectivo y las de tarjeta de crédito pasan a compartir estructura: nombre y saldo juntos en el centro, y un único renglón al pie cuyo contenido es lo que las distingue —tipo de cuenta en unas, barra de utilización en otras—. El avance de gasto del ciclo deja de ser una segunda barra y pasa a un chip junto al saldo, para que la card no cambie de estructura según tenga o no `gasto_minimo_mensual`. La regla toma el siguiente número libre del índice general (RN-331) y no uno del rango original de [[dashboard]]: RN-242 ya estaba en uso por la pestaña Networth de ese mismo documento.|CU-061, CU-063|Ninguno|
