...
- Deploy: Cloudflare Workers (assets-only, `wrangler.jsonc`), auto-deploy en cada push a `main` vía GitHub. Flujo de trabajo vigente por cada ajuste: cambio en código → verificar (`tsc`/`build`/`lint`) → mostrar al usuario → commit → **push solo cuando el usuario lo pida explícitamente** → Cloudflare despliega solo. El commit no necesita autorización (es local y reversible); el push sí, porque en este repo equivale a desplegar a producción — es el último punto de control humano antes de que un cambio llegue a los datos reales del usuario. Nunca empujar por iniciativa propia al terminar de escribir código, ni siquiera con los checks en verde. Migraciones de base de datos **en producción** siempre son manuales (el usuario las corre en el SQL Editor de Supabase), nunca automáticas.
